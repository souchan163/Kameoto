            ...
            int? width = null, int? height = null)
        {
            ...
            screen.Draw(() =>
            {
                for (var i = 0; i < children.Length; i++)
                {
                    ...
                    t.ReferencePoint = KameotoUtil.GetToppedReferencePoint(t.ReferencePoint);
                    var (drawX, _) = KameotoUtil.GetProperPositionFromReferencePoint(texWidth, texHeight, t.ReferencePoint);
                    if (t.ActualSize.Width > texWidth)
                    {
                        // はみ出してしまうのであれば、拡大率を変更する。
                        var r = t.ScaleX;
                        t.ScaleX = KameotoUtil.GetProperScaleX(t, texWidth);
            ...
            screenTex.ScaleY = KameotoUtil.GetProperScaleY(screenTex, height ?? screen.GetTexture().TextureSize.Height);
            ...
            int? width = null, int? height = null)
        {
            ...
            screen.Draw(() =>
            {
                for (var i = 0; i < children.Length; i++)
                {
                    var t = children[i].GetTexture();
                    var origRef = t.ReferencePoint;
                    t.ReferencePoint = KameotoUtil.GetLeftedReferencePoint(t.ReferencePoint);
                    var (_, drawY) = KameotoUtil.GetProperPositionFromReferencePoint(texWidth, texHeight, t.ReferencePoint);
                    if (t.ActualSize.Height > texHeight)
                    {
                        // はみ出してしまうのであれば、拡大率を変更する。
                        var r = t.ScaleY;
                        t.ScaleY = KameotoUtil.GetProperScaleY(t, texHeight);
            ...
            var screenTex = screen.GetTexture();
            screenTex.ScaleX = KameotoUtil.GetProperScaleX(screenTex, width ?? screen.GetTexture().TextureSize.Width);
            result.Draw(() => screenTex.Draw(0, 0));

...
namespace Kameoto.GUI
{
    ...
    public class Scroller : DrawPart
    {
        ...
        public override void Update(bool canHandle, int? pointX = null, int? pointY = null, int parentAbsoluteX = 0, int parentAbsoluteY = 0)
        {
            ...
            {
                // item の矩形を作成（Raylib_cs.Rectangle の場合、各フィールドは float 型です）
                var rect = new Rectangle(item.X, item.Y, item.Width, item.Height);
                bool contains = (MousePoint.x >= rect.X) && (MousePoint.x <= rect.X + rect.Width) &&
                                (MousePoint.y >= rect.Y) && (MousePoint.y <= rect.Y+ rect.Height);

                item.Update(
                    canHandle && !IsOutSide() && contains,

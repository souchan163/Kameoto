...
namespace Kameoto.GUI
{
    ...
    public abstract class DrawPart
    {
        ...
        public DrawPart(int width, int height)
        {
            ...
            LongClickCounter = new Counter(0, Kameoto.LongClickMs - 1, 1000, false);
        ...
        protected bool IsOutSide()
        {
            ...
            // rect.X, rect.Y が左上の座標で、rect.Width, rect.Height がサイズの場合
            return !(MousePoint.x >= rect.X &&
                     MousePoint.x <= rect.X + rect.Width &&
                     MousePoint.y >= rect.Y &&
                     MousePoint.y <= rect.Y + rect.Height);
        }
